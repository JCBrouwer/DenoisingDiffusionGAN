...
def get_activations(files, model, batch_size=50, dims=2048, device="cpu", resize=0):
    ...
    if resize > 0:
        ...
        dataset = ImagePathDataset(
            files, transforms=TF.Compose([TF.Resize(size=(resize, resize)), TF.CenterCrop(resize), TF.ToTensor()])
        )
    else:
        dataset = ImagePathDataset(files, transforms=TF.ToTensor())
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=cpu_count()
    )
...
def calculate_fid_given_paths(paths, batch_size, device, dims, resize=0):
    ...
    m1, s1 = compute_statistics_of_path(paths[0], model, batch_size, dims, device, resize)
    m2, s2 = compute_statistics_of_path(paths[1], model, batch_size, dims, device, resize)
