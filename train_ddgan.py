...
def train(rank, gpu, args):
    ...
    if args.dataset not in ["cifar10", "stackmnist", "lsun", "celeba_256"]:
        ...
        construct_loader = lambda: Loader(
            ...
            pipelines={
                "image": [
                    ...
                    ToTorchImage(),
                    ToFloat(),
                    torchvision.transforms.Normalize([127.5] * 3, [127.5] * 3),
                ]
            },
        )
        ...
        if rebuild:
            ...
            DatasetWriter(
                cache_path, {"image": RGBImageField(max_resolution=max(data[0].shape), jpeg_quality=95)}
    ...
    if args.dataset == "cifar10" or args.dataset == "stackmnist":
        netD = Discriminator_small(
            nc=2 * args.num_channels, ngf=args.ngf, t_emb_dim=args.t_emb_dim, act=nn.LeakyReLU(0.2)
        ).to(device)
    else:
    ...
    for epoch in range(init_epoch, args.num_epoch + 1):
        ...
        with tqdm(data_loader, desc=f"Epoch {epoch}") as pbar:
            for iteration, batch in enumerate(pbar):
                ...
                if iteration % 100 == 0:
                    if rank == 0:
                        pbar.write(f"epoch {epoch} iteration{iteration}, G Loss: {errG.item()}, D Loss: {errD.item()}")
        ...
        if rank == 0:
            torchvision.utils.save_image(
                x_pos_sample, os.path.join(exp_path, "xpos_epoch_{}.png".format(epoch)), normalize=True
            )

            with torch.inference_mode():
                imgs = []
                for _ in range(0, 16 * 9, args.batch_size):
                    x_t_1 = torch.randn_like(real_data)
                    fake_sample = sample_from_model(pos_coeff, netG, args.num_timesteps, x_t_1, T, args)
                    imgs.append(fake_sample.cpu())
                torchvision.utils.save_image(
                    torch.cat(imgs), f"{exp_path}/sample_discrete_epoch_{epoch}.png", nrow=16, normalize=True
                )
